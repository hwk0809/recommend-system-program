...
# 对两种计算下的物品相似度进行加权
def weight_similarity(user_W, tag_W, k):# k为user_W所占比重（0~1）
    W = dict()
    for i, related_items in user_W.items():
        for j, cij in related_items.items():
            W.setdefault(i, {})
            W[i].setdefault(j, 0)
            W[i][j] += k * cij
    for i, related_items in tag_W.items():
        for j, cij in related_items.items():
            W.setdefault(i, {})
            W[i].setdefault(j, 0)
            W[i][j] += (1 - k) * cij
...
user_W = item_similarity(user_data,cluster1)
# W1 = weight_similarity(user_W, tag_W, 0.5)
# W2 = weight_similarity(user_W, tag_W, 0.4)
# result1 = recommend(user_id_list, user_data, W1, 5)
# result2 = recommend(user_id_list, user_data, W2, 5)
# print(result1)
# print(result2)
